...
static CMD_CLASS_GRP  agiTableLifeLineEP1_2[] = {AGITABLE_LIFELINE_GROUP_EP1_2};
static CMD_CLASS_GRP  agiTableLifeLineEP3[] = {AGITABLE_LIFELINE_GROUP_EP3};
...
ApplicationInitSW(void) { /* IN   Nothing */
	...
	AGI_LifeLineGroupSetup(agiTableLifeLineEP3, (sizeof(agiTableLifeLineEP3) / sizeof(CMD_CLASS_GRP)),  GroupName EP_SUPPORT(COMMA ENDPOINT_3));
...
Transmission(
    ...
    if ((COMMAND_CLASS_SWITCH_BINARY == g_NodeList.pCurrentCmdGrp->cmdClass) &&
               (SWITCH_BINARY_REPORT == g_NodeList.pCurrentCmdGrp->cmd)) {
		TRANSMIT_OPTIONS_TYPE_EX pTxOptionsEx;
        pTxOptionsEx.sourceEndpoint = g_NodeList.sourceEndpoint;
        pTxOptionsEx.txOptions = ZWAVE_PLUS_TX_OPTIONS;
        pTxOptionsEx.securityScheme = SEC_SCHEME_AUTO;
        ...
        return CmdClassBinarySwitchReportSendUnsolicited(
                &pTxOptionsEx,
